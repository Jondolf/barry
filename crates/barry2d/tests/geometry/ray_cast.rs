...
use barry2d::query::{Ray, RayCast};
use barry2d::shape::{ConvexPolygon, Segment, Triangle};

#[test]
fn issue_178_parallel_raycast() {
    let m1 = Isometry2::IDENTITY;
    let ray = Ray::new(Vector2::new(0.0, 0.0), Vector2::new(0.0, 1.0));
    let seg = Segment::new(Vector2::new(2.0, 1.0), Vector2::new(2.0, 0.0));
...
fn collinear_raycast_starting_on_segment() {
    let m1 = Isometry2::IDENTITY;
    let ray = Ray::new(Vector2::new(0.0, 0.0), Vector2::new(0.0, 1.0));
    let seg = Segment::new(Vector2::new(0.0, 1.0), Vector2::new(0.0, -1.0));
...
fn collinear_raycast_starting_below_segment() {
    let m1 = Isometry2::IDENTITY;
    let ray = Ray::new(Vector2::new(0.0, -2.0), Vector2::new(0.0, 1.0));
    let seg = Segment::new(Vector2::new(0.0, 1.0), Vector2::new(0.0, -1.0));
...
fn perpendicular_raycast_starting_behind_sement() {
    let segment = Segment::new(Vector2::new(0.0f32, -10.0), Vector2::new(0.0, 10.0));
    let ray = Ray::new(Vector2::new(-1.0, 0.0), Vector2::new(1.0, 0.0));
...
fn perpendicular_raycast_starting_on_segment() {
    let segment = Segment::new(Vector2::new(0.0f32, -10.0), Vector2::new(0.0, 10.0));
    let ray = Ray::new(Vector2::new(0.0, 3.0), Vector2::new(1.0, 0.0));
...
fn perpendicular_raycast_starting_above_segment() {
    let segment = Segment::new(Vector2::new(0.0f32, -10.0), Vector2::new(0.0, 10.0));
    let ray = Ray::new(Vector2::new(0.0, 11.0), Vector2::new(1.0, 0.0));
    assert!(!segment.intersects_local_ray(&ray, std::f32::MAX));
}
...
fn raycast_starting_outside_of_triangle() {
    let triangle = Triangle::new(
        Vector2::new(0.0f32, -10.0),
        Vector2::new(0.0, 10.0),
        Vector2::new(10.0, 0.0),
    );
    let ray = Ray::new(Vector2::new(-10.0, 0.0), Vector2::new(1.0, 0.0));
...
fn raycast_starting_inside_of_triangle() {
    ...
    let ray = Ray::new(Vector2::new(2.0, 0.0), Vector2::new(1.0, 0.0));
    let intersect = triangle
        .cast_local_ray_and_get_normal(&ray, std::f32::MAX, true)
        .expect("No intersection");
...
fn convexpoly_raycast_fuzz() {
    let vertices: Vec<Vector2> = vec![[2, 1], [2, 2], [1, 2], [1, 1]]
        .into_iter()
        .map(|[x, y]| Vector2::new(x as Real, y as Real))
        .collect();

    let square = ConvexPolygon::from_convex_polyline(vertices).unwrap();

    let test_raycast = |ray_origin: Vector2, ray_look_at: Vector2| -> Option<Real> {
    ...
    for i in 0..10_000 {
        let ray_origin = Vector2::new(3., 1. + (i as Real * 0.0001));
        let ray_look_at = Vector2::new(0., 2.);
