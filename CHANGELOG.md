...
- Add validation when using `rkyv` safe API whenever applicable to `barry` types.
...
- Until now, the orientation of the polygon computed by 2D convex hull computation `barry2d::transformation::convex_hull` and
  `barry2d::transformation::convex_hull_idx` wasn't specified (and was generally in clockwise order). Now, this
...
- Add `barry::utils::obb` which computes a (possibly sub-optimal) OBB for a set of points.
...
- Add the ability to use a subset of Barry’s features in a `no-std` context. If the new `cuda` cargo feature of Barry is
